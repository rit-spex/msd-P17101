...
\begin{abstract}
An electrothermal propulsion system was developed for RIT Space Exploration (SPEX) {\color{red}as an exploratory project to develop space systems at the Rochester Institute of Technology such as high-efficiency in-space propulsion for station-keeping maneuvers}.
...
The PCU is housed in a modified mid-tower aluminum computer case shown in~\autoref{fig:pcu-pic}.
...
\begin{figure}[htp]
  \includegraphics[angle=-90,origin=c,width=\linewidth]{figs/pcu-pic.jpg}
  \caption{{\color{red}Add bubbles and label different components.}
\label{fig:pcu-pic}
}
\end{figure}
...
The test stand, shown in~\autoref{fig:teststand-pic}, is cantilevered in design, leveraging both the mass of the thruster and its operating thrust.
...
The propellant solenoid and PCU are activated and deactivated toggle switches next to an emergency shut-off button, shown in~\autoref{fig:pcu-pic}.

\begin{figure}[htp]
  \includegraphics[angle=-90,origin=c,width=\linewidth]{figs/teststand-pic.jpg}
  \caption{{\color{red}Add bubbles and label different components.}
\label{fig:teststand-pic}
}
\end{figure}
...
\begin{figure}
  ...
  \caption{\emph{Far left:} Off.  \emph{Center left:} Ignition. \emph{Center right:} Ionization. \emph{Far right:} Red hot tungsten.
...
\begin{table}
  ...
  \caption{Empirical thrust and $I_{sp}$ for operation in atmosphere, total pressure $p_0= $\SI{239.2}{\kilo\pascal}.
...
\centering
  \begin{tabular}{lrrrrr}
    \toprule
    & \multicolumn{2}{c}{Without arc} & \multicolumn{2}{c}{With arc} \\
    & Thrust\tnote{a} (\si{\milli\newton}) & $I_{sp}$ (\si{\second}) & Thrust (\si{\newton}) & $I_{sp}$ (\si{\second}) \\
    \midrule
    Nitrogen & $12.5$ & $0$ & $ $ & $0$ \\
    Argon    & $-58.1$\tnote{b} & $0$ & $0$ & $0$ \\ \midrule
    Mean     & $-22.8$\tnote{b} & $ $ & $15.0$ & $ $  \\
    \bottomrule
  \end{tabular}
  \begin{tablenotes}
  \item[a] Thrust data is the average tare load (\SI{21.0269}{\newton}) substracted from the average load observed during test fires for each propellant, after filtering.
  \item[b] Negative thrust is a result of significant error from signal noise present in the tare and test data.
  \end{tablenotes}
\end{threeparttable}
\end{table}
