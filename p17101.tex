...
When supplied flow rate exceeded 25 cubic feet per hour (CFH), the exhaust remained partiallyy ionized.
When flow rate was limited to between 22--25\si{CFH}, the exhaust appears to be fully ionized.
...
Using MATLAB software, a null hypothesis of both data sets coming from populations with equal means is rejected at the 1\% significance level by both a standard T-test as well as a Kruskal-Wallis test of variance.
Since the data sets were taken together, this suggests there is a consistent difference between the average thrust measured under each condition.
...
\begin{equation}
  ...
  \end{equation}
This is similar in magnitude to the 200\% gain claimed by previous arcjet thruster systems~\cite{sutton2010rocket}. However, due to the noise present in the system and measurements, this number cannot be claimed as a result and more (and more precise) testing would need to be done to discern if that number is factual.
