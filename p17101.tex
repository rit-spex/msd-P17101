...
\begin{table}[hbp]
  \caption{Theoretical thrust and $I_{sp}$ values with respect to space and sea-level operation, total pressure $p_0= $\SI{137.9}{\kilo\pascal}.
...
While load cell data is relatively clean for the tare and cold fire cases, the data is very noisy while the PCU is engaged.
\autoref{fig:filtered-data} shows the unfiltered and filtered data.

\begin{table}[hbp]
  \caption{Empirical thrust and $I_{sp}$ for sea-level operation, total pressure $p_0= $\SI{137.9}{\kilo\pascal}.
...
\end{table}
\begin{figure}
  \includegraphics[width=\linewidth]{figs/Arcing-Filter.png}
  \caption{}
